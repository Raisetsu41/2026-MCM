...
`main_fast.py` 把此前多级提速版本合并为单文件，仓库只保留两条可运行入口：`main.py`（未提速的基础版本，Q3/Q4 共用同一套证书与状态机）与 `main_fast.py`（提交版本）。合并后用 36 局官方演练与 6 局正式测试的脱敏动作日志做了逐动作回放：42 局的动作序列、位置、频道与虚拟时间全部一致，说明合并没有改变这些案例上的决策。
