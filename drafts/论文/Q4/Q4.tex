% Q4 独立模块稿。合并进主论文时，直接输入“问题四的模型建立与求解”部分即可。
% 参考文献使用主论文 ref.bib 中的 BibTeX key；本章不内嵌 bibliography，避免与 main.tex 重复排版。
...
该变化使 \texttt{no\_signal} 的语义发生改变。
问题三中，它只可能表示频道无源或源距超过接收半径；利用 $r^{\mathrm{recv}}\ge1000\mathrm{m}$ 的先验下界和有限扫描覆盖，可以据此证明“不存在”。
问题四中，定向源仅在朝向闭半平面内辐射，测点位于背面也会返回 \texttt{no\_signal}。
因此，单次 \texttt{no\_signal} 不再是频道为空的充分证据，而只是中性证据。
...
故后续 \texttt{no\_signal} 不能解释为几何故障，也不能假定存在必然可接收的下一测站。

由此，求解路线为：先以方向完备扫描集保证发现；再以正常示向度收缩定位，遇 \texttt{no\_signal} 时优先几何清除、必要时改用有限覆盖清除；最后将扫描站与待服务点合并进行动态开放路径调度。
该路线保留问题三的正证据几何，但改变了 \texttt{no\_signal} 的推理规则。
...
\begin{enumerate}
\item 所有干扰源和检测点均位于同一平面，忽略设备高度差异；干扰源在任务期间保持静止，其频道、位置、有效接收半径和定向方向均不随时间变化。
\item 目标圆域内共有 $N_J$ 个干扰源，$10\le N_J\le16$，但具体个数未知；每个干扰源占用 1 至 20 号频道中的一个不同频道。
\item 干扰源分为全向源和定向源，两种源的个数及定向源方向均未知。全向源的有效覆盖角为 $360^\circ$；第 $j$ 个定向源的发射方向记为 $\boldsymbol o_j$，其有效覆盖角为 $\boldsymbol o_j$ 两侧各 $90^\circ$，且覆盖边界包含在内。
\item 第 $j$ 个干扰源的有效接收半径 $r_j^{\mathrm{recv}}$ 满足 $1000\mathrm{m}\le r_j^{\mathrm{recv}}\le1500\mathrm{m}$。该半径在任务期间不变，且不能直接查询。
\item 定向源的场强在有效覆盖范围内只随距离衰减，不随角度变化。因此，若点 $S$ 到源 $G$ 的距离不超过 $r^{\mathrm{recv}}$，则是否能够检测该源只取决于检测点相对源的位置向量 $S-G$ 是否位于定向方向所定义的闭半平面内。
\item 若测点位于有效覆盖范围内且距离大于 $5\mathrm{m}$，正常示向度与真实方位角之间的误差绝对值不超过 $\delta=1^\circ$。主模型只使用该确定性误差界，不假定误差服从任何概率分布；在线计算使用 $\delta_{\mathrm{num}}=1.01^\circ$ 作为工程安全半角，以吸收接口量化和浮点舍入误差。
\item 若测点与源的距离不超过 $5\mathrm{m}$ 且位于有效覆盖范围内，则检测结果为 \texttt{near}，可直接进入光学精确定位与清除；若位于盲区，则仍为 \texttt{no\_signal}。
\item 光学精确定位和激光清除是否成功只与清除点到干扰源的距离有关，与干扰源是否定向以及定向方向无关。当距离不超过 $r_{\mathrm{opt}}=20\mathrm{m}$ 时清除成功，光学精确定位与清除合计耗时 $5\mathrm{s}$；未发现目标时的清除耗时 $3\mathrm{s}$。
\item 机器狗沿相邻位置间的直线运动，速度为 $5\mathrm{m/s}$；一次检测耗时 $5\mathrm{s}$，一次频道切换耗时 $1\mathrm{s}$。动作按串行顺序执行，请求可安全重试，重试不重复计入时间。
\item 不同频道的信号互不影响，清除一个频道不会改变其他频道的状态；同一地点重复检测的示向误差固定，因此模型不通过同点重复测量消除误差。
\end{enumerate}

\section{符号说明}

本章主要符号如表~\ref{tab:q4_symbols} 所示。

{\setlength{\tabcolsep}{4pt}
\begin{longtable}{@{}>{\small}p{0.20\textwidth}>{\small}p{0.59\textwidth}>{\small}p{0.13\textwidth}@{}}
\caption{问题四主要符号说明}
...
$r^{\mathrm{recv}},r_j^{\mathrm{recv}}$ & 一般干扰源和第 $j$ 个干扰源的有效接收半径 & $1000\sim1500$ m \\
...
设当前任务的路径、检测、换频和清除动作均已确定，总时间仍由式~\eqref{eq:q4_total_time} 给出。
...
\begin{enumerate}
\item 调用 \texttt{/enter} 并记录剩余现实时间，在接口开放后开始任务；
\item 构造 22 点扫描集并执行连续域方向完备认证；若认证失败则回退到解析覆盖集；
\item 将未完成扫描站和待定位、待清除服务点组成动态任务图，求解当前开放路径；
\item 按规划顺序访问扫描站，依次检测尚未闭合的频道；
\item 对返回 \texttt{direction} 的频道记录示向观测，对返回 \texttt{near} 的频道直接清除；
\item 每处理一次正响应后检查已发现的不同频道数；一旦达到 16，立即依据 $N_J\le16$ 将其余 \texttt{unknown} 频道标记为 \texttt{absent\_certified} 并停止访问剩余扫描站；
\item 对已发现频道，用示向锥和外包围继续收缩；当保守半径不超过 $19.999\mathrm{m}$ 时按距离证书清除；
\item 若定位阶段返回 \texttt{no\_signal}，优先使用预算内覆盖清除；预算不足时转入方向完备测站包或带最后单元覆盖证明的清除；
\item 扫描阶段结束时，对所有仍未发现的频道检查方向完备证书，满足条件则标记为 \texttt{absent\_certified}；
\item 删除已清除和已认证为空的频道，重新规划路径，直到 20 个频道全部闭合；
\item 调用 \texttt{/exit}，按定位清除总时间与清除个数计算题面要求的统计值。
\end{enumerate}

\subsection{模型求解与结果分析}

\subsubsection{证书与演练校验}

以下先给出提交版演练与旧版对照，再报告三次正式测试。

使用提交版程序的最近三次演练均正常退出，合计清除 $42/42$ 个实际存在的干扰源，空清除 0 次，平均定位清除时间为 $477.66\mathrm{s}$，最好与最坏分别为 $420.34\mathrm{s}$ 和 $580.98\mathrm{s}$；其中 2 局访问 22 个扫描站，1 局在发现 16 个不同频道后依据 $N_J\le16$ 提前终止，实际访问 21 个扫描站。该批日志中在线决策模块的哈希前 16 位为 \texttt{d5cf45ddcf5065f5}，与三次正式测试一致。
...
三次正式测试均正常退出，结果如表~\ref{tab:q4_formal}所示。表中平均定位清除时间为模拟器的定位清除总时间除以该局清除个数；程序运行时间为模拟器记录的实际运行时间。
...
% 参考文献使用主论文 ref.bib 中的 BibTeX key，由 main.tex 的 \bibliography{ref} 统一生成。
% 本章引用条目如下：
% [wang2019] 王凯. 无线电测向定位方法在新型无线电干扰查找中的运用研究[D]. [出版地不详]: 南昌大学, 2019.
% [zheng2014] 郑春锋. 无线电监测中对干扰源的无源测向定位研究[D]. [出版地不详]: 西南交通大学, 2014.
% [uav2019] WU G. UAV-based interference source localization: A multimodal Q-learning approach[J]. IEEE Access, 2019, 7: 137982-137991.
% [dogancay2005] DOĞANÇAY K. Bearings-only target localization using total least squares[J]. Signal Processing, 2005, 85(9): 1695-1710.
% [bishop2010] BISHOP A N, FIDAN B, ANDERSON B D O, et al. Optimality analysis of sensor-target localization geometries[J]. Automatica, 2010, 46(3): 479-492.
% [sutherland1974] SUTHERLAND I E, HODGMAN G W. Reentrant polygon clipping[J]. Communications of the ACM, 1974, 17(1): 32-42.
