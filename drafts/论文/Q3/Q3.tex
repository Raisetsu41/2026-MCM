% Q3 独立模块稿。合并进主论文时，只输入“问题三的模型建立与求解”部分，避免重复“问题分析、模型假设、符号说明”等全局章节。
% 参考文献使用主论文 ref.bib 中的 BibTeX key；本章不内嵌 bibliography，以免与 main.tex 重复排版。

\section{问题分析}

问题三要求机器狗从原点出发、初始频道为 1。在目标区域内干扰源总数不少于 10 且不多于 16、具体个数未知，且每个干扰源的频道、位置和有效接收半径均未知的条件下，机器狗需完成全部全向干扰源的发现、定位与清除，并使定位清除总时间尽可能短。该问题在问题一、问题二连续几何定位的基础上，进一步加入频道存在性认证、测站信号保证和路径级调度。

首先需要解决的是如何在接收半径未知时严格排除漏源。若仅凭若干次未收到信号就认定频道为空，结论并不可靠。需要先构造一个覆盖整个目标圆域的有限扫描点集，使任一干扰源都必落入某个扫描点的有效接收范围。七点正六边形骨架将最坏最近距离控制在 $968.9016\mathrm{m}$，小于有效接收半径的下界 $1000\mathrm{m}$。因此，若某频道在每个扫描站均有实测 \texttt{no\_signal}，或由严格阴性推断证明该站不能接收该源，就可给出“该频道不存在干扰源”的确定性证书；若已发现 16 个不同频道，则依据题面源数上界，其余频道也可直接认证为空。

其次，单次示向度只能给出带宽度的方向约束，不能直接确定干扰源位置。第一次获得正常示向度后，后续测站必须同时满足“可获得有效响应”和“明显改善定位几何”两项要求。为此，本章利用首次成功观测所蕴含的接收半径下界，构造有信号保证的横向测站；在线几何上沿用问题一的示向锥和半平面交，但采用略大于物理误差界的 $\delta_{\mathrm{num}}=1.01^\circ$，并将所有定位区域保持为真实可行集的凸外包围。只有当外包围的最小覆盖圆半径达到光学清除阈值时，才在圆内选取清除点，从而避免未经证书的试探清除。

最后，总时间由移动、检测、换频和清除时间共同构成。检测和清除的单次耗时固定，而移动时间与路径长度成正比，故优化重点是减少无效往返并合并相邻任务。本章将尚未退役的扫描站和待定位频道的服务点放在同一张动态任务图中，联合优化开放路径；扫描证书只依赖扫描点集合，与访问顺序无关，因此重排路径不会削弱发现证书。路径节点不超过 10 个时，采用动态规划求精确开放路径；节点更多时，采用最小增量插入（cheapest insertion）、2-opt 逆向交换和 Or-opt 邻域重定位进行改善。若正常定位分支停滞，或在按接收保证选取的测站上出现与证书矛盾的 \texttt{no\_signal}，则转入半径上界不超过 $4.5\mathrm{m}$ 的有限单元覆盖并在单元中心检测；单元中心与任一可能源的距离不超过 $4.5\mathrm{m}<5\mathrm{m}$，必然触发近距离清除分支，从而在保持零试探清除的同时给出有限兜底。

综上，本章按“七点覆盖认证、保证信号测站、外包围定位与严格清除判据、动态联合路径调度、模拟验证”的路线展开。无源测向定位研究表明，定位效果受测角误差、接收条件和测站几何的共同影响\cite{wang2019,zheng2014,uav2019}；交会站址的最优性则与目标位置和站源距离密切相关\cite{dogancay2005,bishop2010}。

\section{模型假设}

本章沿用问题一、问题二的平面、静止目标、确定性误差界和无概率分布假设，并针对问题三补充以下假设：

\begin{enumerate}
\item 问题三中的干扰源均为全向干扰源，不存在方向盲区；当测站与干扰源的距离不超过该源的有效接收半径时，必能获得正常示向度或近距离响应。
\item 每个干扰源占用 1 至 20 号频道中的一个频道，不同干扰源的频道互不相同；一局中的干扰源总数 $N_J$ 满足 $10\le N_J\le16$，但具体数值未知。
\item 第 $j$ 个干扰源的有效接收半径 $r_j^{\mathrm{recv}}$ 满足 $1000\mathrm{m}\le r_j^{\mathrm{recv}}\le1500\mathrm{m}$，且在任务期间保持不变。为避免数值边界误判，在线算法将保证接收距离取为 $999.99\mathrm{m}$，将保证超距无信号的距离取为 $1500.01\mathrm{m}$。这两个阈值只吸收接口量化和浮点误差，不改变题面给出的接收半径范围。
\item 示向度真实误差不超过题面给出的 $\delta=1^\circ$。在线计算采用 $\delta_{\mathrm{num}}=1.01^\circ$ 作为工程安全半角，以覆盖接口量化、浮点舍入和边界判定误差；该取值只会扩大示向锥，不会缩小真实可行集。
\item 各干扰源在任务期间保持静止，位置不随时间变化；不同频道之间没有信号串扰，清除一个频道不会影响其他频道。
\item 机器狗沿相邻测站间的直线运动，速度为 $5\mathrm{m/s}$；一次检测耗时 $5\mathrm{s}$，一次换频耗时 $1\mathrm{s}$。清除成功时光学定位与清除合计 $5\mathrm{s}$，清除失败时耗时 $3\mathrm{s}$。
\item 光学装置的作用距离为 $r_{\mathrm{opt}}=20\mathrm{m}$。在线算法使用 $19.999\mathrm{m}$ 作为认证阈值，保留 $0.001\mathrm{m}$ 的数值余量；仅当几何证书成立或检测结果直接为 \texttt{near} 时执行清除。
\item 所有动作按串行顺序执行；相邻动作之间的位置变化产生相应移动时间，测量频道变化计一次换频。
\item 动作请求可安全重试，但重试不改变动作的响应语义，也不重复计入移动、检测或换频成本。
\end{enumerate}

\section{符号说明}

\begin{longtable}{@{}>{\small}p{0.25\textwidth}>{\small}p{0.57\textwidth}>{\small}p{0.12\textwidth}@{}}
\caption{问题三主要符号说明}
...
$P_0$ & 机器狗初始位置与中心扫描点 & $(0,0)$ \\
$P_k$ & 第 $k$ 个正六边形扫描点，$k=1,\ldots,6$ & m \\
$\mathcal S$ & 七点扫描骨架，$\mathcal S=\{P_0,P_1,\ldots,P_6\}$ & 集合 \\
$d_{\max}$ & 目标圆域内任一点到最近扫描点的最大距离 & $968.9016\mathrm{m}$ \\
$c$ & 测向频道编号 & $1,\ldots,20$ \\
$G_j,G$ & 第 $j$ 个干扰源的位置；单源推导时简写为 $G$ & m \\
$N_J$ & 一局中的干扰源总数 & $10,\ldots,16$ \\
$r_j^{\mathrm{recv}},r^{\mathrm{recv}}$ & 第 $j$ 个干扰源的有效接收半径；单源推导时简写为 $r^{\mathrm{recv}}$ & $[1000,1500]\mathrm{m}$ \\
$S_i,S$ & 第 $i$ 个检测点的位置；单站推导时简写为 $S$ & m \\
$\hat\theta_c(S)$ & 在测站 $S$ 对频道 $c$ 测得的示向度 & $(^\circ)$ \\
$\delta,\delta_{\mathrm{num}}$ & 题面误差上界与在线安全半角 & $1^\circ,1.01^\circ$ \\
$d(\phi),d^{\perp}(\phi)$ & 方位角 $\phi$ 的单位方向向量及其左法向量 & 无量纲 \\
$W(S,\hat\theta)$ & 以 $S$ 为顶点、由示向度及安全半角形成的闭示向锥 & 集合 \\
$K_c$ & 频道 $c$ 的真实可行定位区域 & 集合 \\
$A_c$ & $K_c$ 的凸多边形外包围 & 集合 \\
$\mathcal D_c$ & 频道 $c$ 的无信号站集合 & 集合 \\
$R(Q,S)$ & 由“在 $S$ 可接收而在 $Q$ 不可接收”推出的半平面 & 集合 \\
$C_c,\rho_c$ & 外包围 $A_c$ 的覆盖圆心与保守半径上界 & m，m \\
$r_{\mathrm{opt}}$ & 光学精确定位与清除的作用距离 & $20\mathrm{m}$ \\
$\epsilon_{\mathrm{num}}$ & 在线清除半径的数值余量 & $0.001\mathrm{m}$ \\
$S'$ & 首次正常观测后的信号保证横向测站 & m \\
$a_c$ & 小半径收尾阶段十二点测站包的半径 & m \\
$\varepsilon_{\mathrm{cov}}$ & 有限覆盖单元的半径上界 & $4.5\mathrm{m}$ \\
$L$ & 一局中机器狗的总移动距离 & m \\
$N_m,N_s$ & 检测次数与换频次数 & 次 \\
$N_{\mathrm{succ}},N_{\mathrm{miss}}$ & 成功清除实际干扰源的次数与执行空清除的次数 & 次 \\
$N_{\mathrm{clr}}$ & 实际清除的干扰源个数 & 个 \\
$T,\bar T$ & 定位清除总时间与平均定位清除时间 & s，s \\
$\lambda_{\mathrm{clr}}$ & 被清除干扰源个数比例 & 百分数 \\
$V_t,\sigma,n_t$ & 当前重规划中的任务节点集、其开放访问顺序及节点数 & 集合，排列，个 \\
$d_{ij}$ & 任务节点 $i$ 与 $j$ 之间的欧氏距离 & m \\
$P_{\mathrm{cur}}$ & 当前重规划时刻机器狗的位置 & m \\
$M,F_{\mathrm{path}}(M,j)$ & 已访问任务节点集合及对应开放路径的最短距离 & 集合，m \\
\bottomrule
\end{longtable}
...
\qquad
d^{\perp}(\phi)=(-d_y(\phi),\ d_x(\phi))^{\mathsf T}.
\label{eq:q3_direction}
\end{equation}
...
\label{eq:q3_distance}
\end{equation}

定位清除总时间采用

\begin{equation}
T=\frac{L}{5}+5N_m+N_s+5N_{\mathrm{succ}}+3N_{\mathrm{miss}},
\label{eq:q3_total_time}
\end{equation}
...
因此，目标圆域内任一点到 $\mathcal S$ 中最近扫描点的距离不超过 $d_{\max}$。
...
\begin{enumerate}
\item 调用 \texttt{/enter}，记录剩余现实时间并设置截止保护；
\item 初始化七个扫描站和 20 个频道的状态；
\item 利用阴性证据推理退役不可能收到信号的扫描站；若已发现 16 个源，则认证其余频道为空；
\item 将未退役扫描站和待服务频道组成动态节点集，求联合开放路径；
\item 若下一节点为扫描站，则测量尚未闭合的频道，并在有严格收益时执行站内顺路收获；
\item 若下一节点为服务频道，则按保证信号测站、误差上界测站和十二点测站包的顺序继续定位；
\item 当外包围覆盖半径不超过 $19.999\mathrm{m}$ 时，在满足距离证书的保守覆盖圆内选择清除点；若检测为 \texttt{near}，就地清除；
\item 若正常定位分支停滞，或保证测站出现与证书矛盾的 \texttt{no\_signal}，则细分当前外包围至半径不超过 $4.5\mathrm{m}$，在叶单元中心检测；所测叶单元中心与真实源的距离不超过 $4.5\mathrm{m}$，必然触发 \texttt{near} 清除；
\item 删去已清除与已认证为空的频道，重复路径规划和执行，直到 20 个频道均有确定状态；
\item 调用一次 \texttt{/exit}，输出清除数、检测次数、换频次数、总虚拟时间和证书状态。
\end{enumerate}
...
使用提交版程序的最近三次演练共清除 39 个干扰源，清除失败 0 次，平均定位清除时间为 $268.62\mathrm{s}$，最好与最坏分别为 $255.35\mathrm{s}$ 和 $293.50\mathrm{s}$。该批演练均正常退出。

此前 15 局快速版迭代演练共清除 201 个源，平均时间为 $262.23\mathrm{s}$，中位数为 $259.59\mathrm{s}$，最好与最坏分别为 $197.43\mathrm{s}$ 和 $325.96\mathrm{s}$，空清除次数为 0。该批日志中在线决策模块的哈希前 16 位为 \texttt{2704db2749dd7700}，与提交版不同，故只作迭代前后对照，不与正式版样本合并统计。

早期版本的官方演练对照中，立即调度 18 局的平均时间为 $507.91\mathrm{s}$/源，批量调度 7 局降至 $418.52\mathrm{s}$/源，降幅为 $17.60\%$。

进一步引入联合选路、站内顺路收获、带误差上界的局部测站和十二点测站包后，三次正式测试单局平均值的算术平均降至 $289.91\mathrm{s}$/源。该对照说明时间收益主要来自减少重复往返和无效测站，而不是减少必要的几何认证。

\subsubsection{正式测试结果}

三次正式测试均正常退出，且与最近三次提交版演练使用同一组四文件哈希，其中在线决策模块的 SHA-256 前 16 位为 \texttt{d5cf45ddcf5065f5}。结果如表~\ref{tab:q3_formal}所示。
...
\midrule
T5JT-TE93-CQUC-FDAD & 12 & 384.94 & 2402 \\
