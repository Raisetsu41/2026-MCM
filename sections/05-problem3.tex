% 问题三
\section{问题三：全向多源的自主定位与清除}

\subsection{模型准备}

\subsubsection{动作协议与时间模型}

设路径依次访问 $P_0,P_1,\ldots,P_M$，问题三的虚拟时间为
\begin{equation}
...
并将目标圆域按六个 $60^\circ$ 扇区划分，第 $k$ 个扇区以 $P_k$ 为中心方向。对扇区内任一点 $G$，记 $r=\lVert G\rVert_2$，其相对 $P_k$ 的夹角为 $\varphi\in[-30^\circ,30^\circ]$。由于构型关于每个扇区的中线对称，只需分析 $0\le\varphi\le30^\circ$；六个扇区除旋转外完全相同。
...
证明：由 $d_{\max}<1000\mathrm{m}$，任一干扰源到最近扫描点的距离不超过 $968.9016\mathrm{m}$，位于其有效接收范围内。由于问题三中的干扰源为全向源，该点不会出现方向盲区，因此测量结果只能是 \texttt{direction} 或 \texttt{near}。反之，若七个点均无信号，则与上述必然有信号矛盾。证毕。\par\vspace{1em}
...
\]
因此 $\lVert G-S'\rVert_2<1000\mathrm{m}\le r^{\mathrm{recv}}$。由以上两种距离情形，$S'$ 必能收到该源的有效响应。该构造只改变测量位置，不增加误差分布假设。

\subsubsection{定位区域更新与测站选择}

初始化频道 $c$ 的外包围为覆盖目标圆域的正多边形。记 $\mathcal D_c$ 为频道 $c$ 已测得无信号的检测点集合，并以 $R(Q,S)$ 表示式~\eqref{eq:q3_negative_halfplane} 由无信号站 $Q$ 与正常测站 $S$ 确定的开半平面。获得正常示向度后，依次执行：
...
由发现覆盖、定位外包围和有限覆盖三项证书，在本章假设下，每个实际存在的全向干扰源都能被发现并最终获得合法清除路径；每个未发现频道也都有实测 \texttt{no\_signal}、严格阴性推断或 16 源上界之一作为认证依据。因此，终止时不会出现未闭合频道。
...
$L(\sigma)/5$ 只是当前节点集冻结时总虚拟时间中的移动部分；访问顺序还可能改变后续检测、换频和清除次数。因此每次只执行路径首节点，更新频道证据和节点集后重新规划，最终耗时仍按式~\eqref{eq:q3_time} 核算。
...
节点数超过 10 时，Held--Karp 状态数随 $2^{n_t}$ 增长，故先以最小增量插入构造初始解，并加入 3 个最近邻起点，再交替执行 2-opt 逆向交换和 Or-opt 邻域重定位直至无改善。此时只声称得到高质量可行路径，不声称是全局最优解。
...
未提速基础版取调度对照数据中的 \texttt{immediate} 组，共完成 18 局演练，平均定位清除时间为 $507.91\mathrm{s}$/源，所有实际干扰源均被清除且清除失败为 0；另有 7 局 \texttt{batch} 组只用于早期调度对照，不并入基础版或完全提速版统计。
...
令 $N_{\mathrm{clr}}$、$N_J$ 与 $V_{\mathrm{clr}}$ 分别表示被清除干扰源个数、实际干扰源总数与定位清除虚拟总时间。按题面定义，清除比例和平均定位清除时间分别为 $\lambda_{\mathrm{clr}}=N_{\mathrm{clr}}/N_J$ 与 $\bar t_{\mathrm{clr}}=V_{\mathrm{clr}}/N_{\mathrm{clr}}$。三次正式测试与其同期演练使用同一构建，提交版本在去冗余注释、文件名版本对齐与输出消息英文字符化后哈希为 \texttt{d936031c54075554}（在线决策模块 SHA-256 前 16 位）。结果如表~\ref{tab:q3_formal}所示。
...
FX3P-PUBX-QE4N-783K & 13 & 220.49 & 2198 \\
Y4HS-EJMD-CXRP-BV5W & 15 & 264.31 & 2404 \\
\midrule
合计／总体平均／区间 & 40 & 286.26 & 2198--2404 \\
\bottomrule
\end{tabular}
\end{table}

表中汇总行的 40 为三次清除数之和。按题面合并口径，每个被清除干扰源的平均定位清除时间为 $11450.33/40=286.26\mathrm{s}$，程序运行时间列给出三次取值区间。三次单局指标的等权算术平均为 $289.91\mathrm{s}$/源，仅作为案例间对照。
三次正式测试共清除 40 个干扰源，三次均无空清除。每局的 20 个频道最终均为已清除或认证为空状态，没有未知频道残留。由命题 1 的发现完备性，所有被判为不存在的频道必无干扰源；因此三次测试均有 $N_{\mathrm{clr}}=N_J$，合并清除比例为 $\lambda_{\mathrm{clr}}=40/40=100\%$。
