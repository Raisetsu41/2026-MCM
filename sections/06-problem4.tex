...
\subsubsection{统一闭环与求解算法}

问题三与问题四共享“认证状态---生成任务---联合选路---执行首节点---更新证据”的控制闭环，见图~\ref{fig:q34_closed_loop}。该图只表达不会随具体几何策略改变的控制骨架；两问对 \texttt{no\_signal} 的不同解释及各自兜底过程由后续伪代码给出。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  box/.style={rectangle,rounded corners,draw=black,align=center,minimum height=8mm,text width=9.6cm,font=\normalsize},
  check/.style={rectangle,rounded corners,draw=black,fill=black!5,align=center,minimum height=8mm,text width=5.6cm,font=\normalsize},
  line/.style={-Latex,thick}
]
  \node[box] (init) at (0,0) {验证扫描集证书，初始化 20 个频道状态与观测集};
  \node[box] (tasks) at (0,-1.10) {认证已有状态，生成未完成扫描站与频道服务点};
  \node[box] (route) at (0,-2.20) {求联合开放路径，仅执行当前首节点};
  \node[box] (update) at (0,-3.30) {吸收动作响应，更新可行域、频道证书与当前位置};
  \node[check] (closed) at (0,-4.40) {20 个频道是否全部闭合？};
  \node[check] (done) at (0,-5.50) {一致性核验通过，调用一次 \texttt{/exit}};
  \draw[line] (init)--(tasks);
  \draw[line] (tasks)--(route);
  \draw[line] (route)--(update);
  \draw[line] (update)--(closed);
  \draw[line] (closed)--node[right]{是}(done);
  \draw[line] (closed.west) to[out=180,in=180,looseness=1.25]
    node[left]{否} (tasks.west);
\end{tikzpicture}
\caption{问题三、四共享的证据驱动在线控制主循环}
\label{fig:q34_closed_loop}
\end{figure}
...
1 \> 连续域认证 $P_4'$；若认证失败，则改用已证明方向完备的 25 点扫描集；\\
