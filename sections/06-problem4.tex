...
程序核验表明，该集合在 7372 个单元内完成认证；一旦达到限制仍未完成，程序拒绝该证书并转入带解析参数检查的 25 点双环工程回退分支，参数检查不通过时停止执行。该回退分支在本文 18 局完全提速版演练和三次正式测试中均未触发。
...
1 \> 连续域认证 $P_4'$；若认证失败，则进入 25 点双环工程回退分支并检查其解析参数；\\
